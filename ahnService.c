	...
	while (isMuxReading){
        ...
        if(ret > HEAD_LEN_IP_TRANS){
			...
			if((read_buf[0]==0x55)&&(read_buf[1]==0x10))
			{
				...
				strncpy(buf_remove_head,read_buf + HEAD_LEN_IP_TRANS,frame_size);
				struct ip *iph           = NULL;
	...
	while(1) {
        ...
        switch(ether_type) {
            case ETHERTYPE_ARP: {
				...
				{
					char buf_add_flag[s_frame_size + HEAD_LEN_IP_TRANS];
					buf_add_flag[0] = 0x55;
					buf_add_flag[1] = 0x09;
				
					//add head
					strcat(buf_add_flag,(char *)s_frame_data);
					#if xdebug
						printf("+++++++++++++++++++recieve data ARP+++++++++++++++++++++++++++\n");
						printf("========frame size:%d\n", s_frame_size);
						dump_frame_ether(eth);
						dump_frame_arp  (arp);
